...
def voltage_to_resistance_critCurr(v_obs_uv,bias_r=1E6,v_mon = 0.939E-3): 
    '''
    DESC: this function converts output of the 
    INPUT: 
        v_obs_uv: the measured voltage from the probe statation, units uV
        bias_r   : bias resistor used in resistance measurement
        v_mon   : voltage of probe station monitor in volts
    '''
...
def parse_input(input_list, regex_template, num_blocks,num_cols=None):
    '''
    A function to extract all voltage measurements from a list of file lines 
    copied from oneNote
    input_list: string list of rows of input file
